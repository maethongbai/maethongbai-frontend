        ...
        <div class="border">
            ...
            <div class="pl-5">
                <table class="border w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" v-for="gold in selled_gold">
                        <tr >
                            <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{gold.id}}</td>
                            <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{gold.id}}</td>
                            <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{gold.id}}</td>
                        </tr>
                    </tbody>
...
import { useOnsiteSaleStore } from '@/stores/onsiteSale.js'
import { useOnlineSaleStore } from '@/stores/onlineSale.js'
import { useGoldStore } from '@/stores/gold.js'
import { useAllSaleStore } from '@/stores/all_sale.js'
...
export default {
    ...
    async mounted() {
        ...
        var today = new Date("2013-07-25")
        //console.log(today)
        var test = new Date(moment().format("YYYY-MM-DD"))
        //console.log(test)
        
        await this.onlineSale_store.fetch()
        // this.onlineSales_not_delivery = this.onlineSale_store.getOnlineSales
        // console.table(this.onlineSales_not_delivery)
        this.onlineSales_not_delivery = this.onlineSale_store.getAcceptFromDate(today)
        console.table(this.onlineSales_not_delivery)

        await this.gold_store.fetch()
        this.selled_gold = this.gold_store.countSoldGold()
